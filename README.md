...
├── .updator/
│   ├── shared.json             # zap-publish + client 共用配置（会上传到 server）
...
client.yaml 保留在 `zap-update.exe` 同级目录（UpdateFolder），仅包含定位 ApplicationFolder 所需的字段：
...
在 `zap-update.exe` 同级目录创建 `client.yaml`（精简版）：
...
1. 读取 `client.yaml`（与 `zap-update.exe` 同级目录）→ 获取 `main_exe_relative_path`
...
```bash
zap-update.exe apply_update [--main-exe-path <path>] [--must-close-process-name <n1,n2>] [--close-timeout <seconds>]
```
...
```bash
zap-update.exe rollback --version 1.0.0 [--must-close-process-name <n1,n2>] [--close-timeout <seconds>]
```
...
# 4. 应用更新（会重启应用）
zap-update.exe apply_update --must-close-process-name "myapp,myapp-plugin" --close-timeout 30
