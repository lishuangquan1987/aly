...
一个轻量级的客户端自动更新解决方案，包含四个模块：server（服务端）、publish-cli（发布引擎）、publish-gui（发布 GUI）、client（终端用户更新器）。
...
| 调用方 | 被调用方 | 通信方式 | 数据格式 |
|--------|---------|---------|---------|
| publish-gui | publish-cli | 子进程（`ProcessService`） | stdout JSON（`--json` 参数） |
| publish-cli | server | HTTP REST API | JSON（camelCase 统一包装） |
| client | server | HTTP REST API | JSON（camelCase 统一包装） |

publish-gui 本身 **不直接调用 server API**，所有发布操作都通过子进程调用 publish-cli 完成。publish-gui 仅在本地维护一个项目列表配置（`%LOCALAPPDATA%/PublishGui/config.json`）。

**唯一例外**：添加项目时，publish-gui 调用 `publish-cli config init` 初始化本地项目配置，并通过 `publish-cli project list` / `publish-cli project create` 操作服务端项目。

---

## `.updator/` 共享配置体系

publish-cli 和 client 共用一套配置，存放在构建产物目录的 `.updator/` 文件夹中（类似 `.git` 的概念）。

### 目录结构

```
构建产物目录/                    ← publish-cli 的 project_path
├── .updator/
│   ├── shared.json             # publish-cli + client 共用配置（会上传到 server）
│   ├── client.json             # client 专有配置（会上传到 server）
│   ├── publish.json            # publish-cli 专有配置（本地，不上传）
│   └── staging/
...
| `post_update_script` | `string` | 更新后执行的脚本路径（相对于 ApplicationFolder） |

### publish.json（publish-cli 专有，本地不上传）
...
> `project_id` 不需要记录，因为 `shared.json` 中已有 `project_name`，publish-cli 通过项目名称查找即可定位 server 上的项目。

### 哪些文件会上传到 server

publish-cli 在扫描文件时自动忽略 `.updator/` 目录。此外，以下文件不会出现在 server 的文件清单中：

- `.updator/` 整个目录（scanner 自动跳过）
- `.publish-cli/` 整个目录（向后兼容，scanner 自动跳过）

`shared.json` 和 `client.json` 会随构建产物一起上传到 server。当 client 下载更新时，新版本的 `shared.json` 和 `client.json` 也会被下载到 `ApplicationFolder/.updator/` 中，使客户端配置自动跟随版本更新。

> **注意**：`shared.json` 中的 `ignore_folders` / `ignore_files` 同时用于两个场景：
> 1. **publish-cli 扫描** — 决定哪些文件被扫描并上传到 server
> 2. **client apply_update** — `CopyDirWithExclude` 时决定哪些文件/文件夹不复制到版本目录

### client.yaml（精简版）

client.yaml 保留在 `client-updator.exe` 同级目录（UpdateFolder），仅包含定位 ApplicationFolder 所需的字段：
...
client 启动时的配置加载流程：
1. 读取 `client.yaml` → 获取 `main_exe_relative_path`
2. 通过 `main_exe_relative_path` 推导出 `ApplicationFolder` 路径
3. 读取 `ApplicationFolder/.updator/shared.json` → 获取 `server_url`、`project_name` 等
4. 读取 `ApplicationFolder/.updator/client.json` → 获取 `must_close_process_name` 等
5. 命令行参数覆盖以上所有配置

### 配置优先级

**publish-cli**：CLI 参数 > `.updator/shared.json` + `.updator/publish.json`

**client**：CLI 参数 > `ApplicationFolder/.updator/client.json` + `ApplicationFolder/.updator/shared.json` > `client.yaml`
...
在 `client-updator.exe` 同级目录创建 `client.yaml`（精简版）：

```yaml
main_exe_relative_path: "../ApplicationFolder/main_application.exe"
```

在 ApplicationFolder 中创建 `.updator/shared.json` 和 `.updator/client.json`（或通过 publish-cli 发布时自动带入）。
...
└── UpdateFolder/                       # 更新程序目录
    ├── client-updator.exe
    ├── client.yaml                     # 精简配置（仅 main_exe_relative_path）
    ├── version.json                    # 版本状态机
    └── logs/
```
...
| `UpdateFolder/` | 部署时手动放置 | client-updator.exe 及其配置 |
...
类 Git 工作流的命令行工具，是整个发布流程的核心引擎。publish-gui 通过子进程调用它完成所有发布操作。
...
| `--path` | 本地构建产物路径 | CLI 参数（必传），publish-gui 的本地配置 |
...
```bash
publish-cli project list --server http://localhost:2000
```
...
```bash
publish-cli project update --name myapp --title "新标题" --force-update
```
...
```bash
publish-cli project delete --name myapp
```
...
```bash
publish-cli project info --name myapp
```
...
2. `diff.RunStatus()` 执行核心比对：
   - `diff.ScanDirectory()` 递归扫描本地目录（自动跳过 `.updator/` 和 `.publish-cli/`，应用 ignore 规则），对每个文件同时计算 MD5 和 SHA256
...
```bash
publish-cli add app.exe lib.dll    # 添加指定文件
publish-cli add --all              # 添加所有 new + modified 文件
```
...
```bash
publish-cli reset app.exe     # 移除指定文件
publish-cli reset --all       # 清空暂存区
```
...
```bash
publish-cli push --version V1.0.1 --message "修复登录bug" --message "新增搜索"
```
...
```bash
publish-cli watch --interval 5 --auto-add
```
...
```bash
publish-cli server info --project myapp
```
...
1. 读取 `client.yaml`（与 `client-updator.exe` 同级目录）→ 获取 `main_exe_relative_path`
...
4. 读取 `ApplicationFolder/.updator/client.json` → 获取 `must_close_process_name`、`post_update_script`
5. 命令行参数覆盖以上所有配置（`--url`、`--project-name`、`--main-exe-path`、`--must-close-process-name`）

### JSON 输出格式

所有命令输出 JSON 到 stdout（与 publish-cli 格式统一）：
...
3. 调 `FindProjectByName()` → `GET /api/project/get_all_projects` → 按 name 查找 → 获取 `project.id` 和 `project.force_update`
4. 调 `GetProjectChangeLogs()` → `GET /api/project/get_project_change_logs/{id}` → 取 ID 最大的日志 → 获取 `latestVersion`
...
**实现**：
1. 获取最新版本号（`FindProjectByName()` → `GetProjectChangeLogs()` → 取最新）
...
```bash
client-updator.exe apply_update [--main-exe-path <path>] [--must-close-process-name <n1,n2>] [--close-timeout <seconds>]
```
...
**3. 关闭目标进程**：
- 对 `must_close_process_name` 中的每个进程名：
...
**6. 执行 `post_update_script`**（如果配置了，异步执行）
...
```bash
client-updator.exe rollback --version 1.0.0 [--must-close-process-name <n1,n2>] [--close-timeout <seconds>]
```
...
**实现**：
1. 计算自身文件（`client-updator.exe`）的 SHA256
2. 检查 `ApplicationFolder/check-updator.exe` 是否存在（不存在 → 首次部署，无需自更新）
3. 计算 `check-updator.exe` 的 SHA256
...
version.json 位于 UpdateFolder（`client-updator.exe` 同级目录）：
...
- 当检测到 `version_status = "applying"` 时：
  ...
  - 两种恢复路径都会执行 `post_update_script` 并启动主程序
...
面向发布者的桌面工具（Avalonia 12），纯前端壳子，所有发布操作通过子进程调用 publish-cli 完成。
...
- 存储位置：`%LOCALAPPDATA%/PublishGui/config.json`
...
**CliService** — publish-cli 命令封装

- 自动查找 `publish-cli.exe`（同级目录 → 相对路径回退）
...
|---------|---------------|----------------|
| **刷新** | `RefreshAsync()` | `publish-cli status --json --path "{path}"` + `publish-cli log --limit 20 --json --path "{path}"` |
| **全部暂存** | `AddAllAsync()` | `publish-cli add --all --json --path "{path}"` |
| **清空暂存** | `ResetAllAsync()` | `publish-cli reset --all --json --path "{path}"` |
| **暂存选中文件** | `AddSelectedAsync()` | `publish-cli add "file1" "file2" ... --json --path "{path}"` |
| **取消暂存选中** | `ResetSelectedAsync()` | 先 `publish-cli reset --all --json --path "{path}"`，再 `publish-cli add "保留文件1" ... --json --path "{path}"` |
| **发布** | `PublishAsync()` | `publish-cli push --version "{ver}" --message "{msg}" --json --path "{path}"`（timeout 120s） |
| **添加项目** | `AddProjectAsync()` | 打开 AddProjectDialog → 选择已有项目或创建新项目 → `publish-cli config init --server "{url}" --project "{name}" --path "{path}" --json` + `publish-cli project list --server "{url}" --json` 或 `publish-cli project create --server "{url}" --name "{name}" --title "{title}" ... --json` |
| **移除项目** | `RemoveProjectAsync()` | 仅操作本地 ConfigService，不调用 CLI |

### 发布流程（GUI 用户视角）

1. 用户启动 publish-gui → 加载 `%LOCALAPPDATA%/PublishGui/config.json` 中的项目列表
2. 选择项目 → 自动调 `status` + `log` 刷新数据
3. 左侧显示 unstaged 文件列表（new/modified/deleted）
4. 右侧显示 staged 文件列表
5. 用户勾选文件 → 点击"暂存选中" 或 "全部暂存"
6. 输入版本号和变更说明 → 点击"发布"
7. GUI 调 `push` → publish-cli 上传文件 + 创建版本 → 清空暂存区 → 自动刷新
...
{
    private readonly string _updatorPath; // client-updator.exe 的路径
...
public static class AppStartup
{
    ...
    {
        var updater = new UpdateManager("UpdateFolder/client-updator.exe");
...
# 2. 检查更新
client-updator.exe check_update
# → {"isSuccess":true,"errorMsg":"","data":{"has_update":true,"current_version":"1.0.0","new_version":"1.0.1","force_update":false}}

# 3. 下载更新
client-updator.exe download_update
# → {"isSuccess":true,"errorMsg":"","data":{"version":"1.0.1"}}

# 4. 应用更新（会重启应用）
client-updator.exe apply_update --must-close-process-name "myapp,myapp-plugin" --close-timeout 30
