...
1. **`aly-client.exe`**（位于 `client/aly-client/`）— 自动更新客户端，负责检查、下载、应用更新及回滚
2. **`aly-publish.exe`**（位于 `publish/publish-cli/`）— 发布命令行工具，负责项目管理、文件比对、暂存和推送发布

---

## 一、更新客户端 — aly-client.exe
...
```bash
aly-client.exe check_update [--url <服务器地址>] [--project-name <项目名称>]
```
...
```bash
aly-client.exe check_update --url http://192.168.1.100:2000 --project-name myapp
```
...
```bash
aly-client.exe check_diff [--url <服务器地址>] [--project-name <项目名称>]
```
...
```bash
aly-client.exe check_diff
```

**示例 2：指定远程项目查看差异**

```bash
aly-client.exe check_diff --url http://update.example.com:2000 --project-name game_client
```
...
```bash
aly-client.exe download_update [--url <服务器地址>] [--project-name <项目名称>] [--main-exe-path <相对路径>]
```
...
```bash
aly-client.exe download_update --url http://192.168.1.100:2000 --project-name myapp --main-exe-path ../MyApp/MyApp.exe
```
...
```bash
aly-client.exe apply_update [--main-exe-path <路径>] [--close-timeout <秒数>]
```
...
```bash
aly-client.exe apply_update --close-timeout 60
```
...
```bash
aly-client.exe list_rollback_versions [--main-exe-path <路径>]
```
...
```bash
aly-client.exe list_rollback_versions
```

**示例 2：指定主程序路径查看历史版本**

```bash
aly-client.exe list_rollback_versions --main-exe-path ../MyApp/MyApp.exe
```
...
```bash
aly-client.exe rollback --version <目标版本号> [--main-exe-path <路径>] [--close-timeout <秒数>]
```
...
```bash
aly-client.exe rollback --version 1.0.0
```

**示例 2：回滚到指定版本并延长超时**

```bash
aly-client.exe rollback --version 0.9.0 --close-timeout 45
```
...
**用途**：比对更新客户端自身（`aly-client.exe`）是否需要更新。
...
```bash
aly-client.exe check_self_update [--main-exe-path <路径>]
```
...
1. 找到 `ApplicationFolder/aly-client.exe`（随每次发布打包的副本）
2. 计算当前运行中的 `aly-client.exe`（`UpdateFolder/aly-client.exe`）的 SHA256
3. 计算 `ApplicationFolder/aly-client.exe` 的 SHA256
4. 若两者不一致 → 需要更新
5. 若 `ApplicationFolder/aly-client.exe` 不存在（首次部署）→ 不需要更新

> **原理**：发布者每次打包时将当前版本的 `aly-client.exe` 复制一份放入主程序目录。更新后主程序目录的版本已更新，但 UpdateFolder 的仍是旧版。
...
```bash
aly-client.exe check_self_update
```

**示例 2：指定主程序路径进行自检**

```bash
aly-client.exe check_self_update --main-exe-path ../MyApp/MyApp.exe
```

---

## 二、发布命令行工具 — aly-publish.exe
...
**使用场景**：首次使用 `aly-publish` 发布项目时的初始化步骤。

**用法**：

```bash
aly-publish config init --project <项目名称> [--server <服务器地址>]
```
...
```bash
cd /path/to/myapp
aly-publish config init --project myapp --server http://192.168.1.100:2000
```
...
```bash
cd /path/to/myapp
aly-publish config init --project myapp
aly-publish config set server.url http://192.168.1.100:2000
```
...
```bash
aly-publish config set <key> <value>
```
...
```bash
aly-publish config set-array <key> --add <条目> | --remove <条目> | --clear
```
...
```bash
aly-publish config set-array ignore.folders --add logs
aly-publish config set-array ignore.folders --add temp
```

**示例 2：移除忽略文件夹并查看结果**

```bash
aly-publish config set-array ignore.folders --remove temp
...
```bash
aly-publish config set-array ignore.files --clear
```
...
```bash
aly-publish config get project.name
```
...
```bash
aly-publish project list [--server <地址>]
```
...
```bash
aly-publish project list --server http://192.168.1.100:2000
```

**示例 2：JSON 格式输出**

```bash
aly-publish project list --server http://192.168.1.100:2000 --json
```
...
```bash
aly-publish project create --name <名称> --title <标题> [--force-update] [--ignore-folders <逗号分隔>] [--ignore-files <逗号分隔>]
```
...
```bash
aly-publish project create --name myapp --title "我的应用"
```

**示例 2：创建带忽略规则的项目**

```bash
aly-publish project create --name game_client --title "游戏客户端" --force-update --ignore-folders "logs,cache" --ignore-files "*.log,.DS_Store"
```
...
```bash
aly-publish project update --name <名称> [--title <标题>] [--force-update] [--ignore-folders <逗号分隔>] [--ignore-files <逗号分隔>]
```
...
```bash
aly-publish project update --name myapp --title "我的应用 V2"
```

**示例 2：开启强制更新**

```bash
aly-publish project update --name myapp --force-update
```
...
```bash
aly-publish project delete --name <名称>
```
...
```bash
aly-publish project delete --name old_project
```
...
```bash
aly-publish project info [--name <名称> | --id <ID>]
```
...
```bash
aly-publish project info --id 1 --json
```
...
```bash
aly-publish status [--server <地址>] [--project <名称>]
```
...
Changes staged for commit:
  (use "aly-publish reset <file>..." to unstage)

        modified:    src/main.go
        new:         config/new_setting.json

Changes not staged for commit:
  (use "aly-publish add <file>..." to stage)
...
```bash
cd /path/to/myapp
aly-publish status --server http://192.168.1.100:2000 --project myapp
```

**示例 2：JSON 格式输出差异**

```bash
aly-publish status --server http://192.168.1.100:2000 --project myapp --json
```
...
```bash
aly-publish diff --file src/main.go
```
...
```bash
aly-publish add [--all | <file1> <file2> ...]
```
...
```bash
aly-publish add src/main.go config/settings.json
```
...
```bash
aly-publish reset [--all | <file1> <file2> ...]
```
...
```bash
aly-publish reset --all
```

**示例 2：移除指定文件**

```bash
aly-publish reset config/settings.json
```
...
```bash
aly-publish push --version <版本号> --message <变更说明> [--dry-run] [--force]
```
...
```bash
aly-publish push --version 1.0.1 --message "修复登录闪退" --message "优化首页加载速度"
```

**示例 2：使用 dry-run 先行验证**

```bash
aly-publish push --version 1.0.1 --message "修复 bug" --dry-run
```
...
```bash
aly-publish log --limit 5
```

**示例 2：JSON 格式输出全部日志**

```bash
aly-publish log --limit 100 --json
```
...
```bash
aly-publish server info [--server <地址>]
```
...
```bash
aly-publish server info --server http://192.168.1.100:2000
```

**示例 2：JSON 格式输出**

```bash
aly-publish server info --server http://192.168.1.100:2000 --json
```
...
# 步骤 5：推送到服务端
aly-publish push --version 1.0.0 --message "首次发布"

# 或者使用一键发布
aly-publish publish --version 1.0.0 --message "首次发布"
```
