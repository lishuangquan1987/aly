...
1. **`zap-client.exe`**（位于 `client/zap-client/`）— 自动更新客户端，负责检查、下载、应用更新及回滚
2. **`zap-publish.exe`**（位于 `publish/publish-cli/`）— 发布命令行工具，负责项目管理、文件比对、暂存和推送发布

---

## 一、更新客户端 — zap-client.exe
...
```bash
zap-client.exe check_update [--url <服务器地址>] [--project-name <项目名称>]
```
...
```bash
zap-client.exe check_update --url http://192.168.1.100:2000 --project-name myapp
```
...
```bash
zap-client.exe check_diff [--url <服务器地址>] [--project-name <项目名称>]
```
...
```bash
zap-client.exe check_diff --url http://update.example.com:2000 --project-name game_client
```
...
```bash
zap-client.exe download_update [--url <服务器地址>] [--project-name <项目名称>] [--main-exe-path <相对路径>]
```
...
```bash
zap-client.exe download_update --url http://192.168.1.100:2000 --project-name myapp --main-exe-path ../MyApp/MyApp.exe
```
...
```bash
zap-client.exe apply_update [--main-exe-path <路径>] [--close-timeout <秒数>]
```
...
```bash
zap-client.exe apply_update --close-timeout 60
```
...
```bash
zap-client.exe list_rollback_versions [--main-exe-path <路径>]
```
...
```bash
zap-client.exe list_rollback_versions --main-exe-path ../MyApp/MyApp.exe
```
...
```bash
zap-client.exe rollback --version <目标版本号> [--main-exe-path <路径>] [--close-timeout <秒数>]
```
...
```bash
zap-client.exe rollback --version 1.0.0
```

**示例 2：回滚到指定版本并延长超时**

```bash
zap-client.exe rollback --version 0.9.0 --close-timeout 45
```
...
**用途**：比对更新客户端自身（`zap-client.exe`）是否需要更新。
...
```bash
zap-client.exe check_self_update [--main-exe-path <路径>]
```
...
1. 找到 `ApplicationFolder/zap-client.exe`（随每次发布打包的副本）
2. 计算当前运行中的 `zap-client.exe`（`UpdateFolder/zap-client.exe`）的 SHA256
3. 计算 `ApplicationFolder/zap-client.exe` 的 SHA256
4. 若两者不一致 → 需要更新
5. 若 `ApplicationFolder/zap-client.exe` 不存在（首次部署）→ 不需要更新

> **原理**：发布者每次打包时将当前版本的 `zap-client.exe` 复制一份放入主程序目录。更新后主程序目录的版本已更新，但 UpdateFolder 的仍是旧版。
...
```bash
zap-client.exe check_self_update --main-exe-path ../MyApp/MyApp.exe
```
