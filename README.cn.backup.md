...
一个轻量级的客户端自动更新解决方案，包含四个模块：aly-server（服务端）、aly-publish（发布引擎，原 aly-publish）、AlyPublish（发布 GUI）、aly-update（终端更新器）。
...
├── publish/
│   ├── aly-publish-gui/       # 发布工具 GUI（Avalonia 12 桌面应用）
│   └── aly-publish/           # 发布工具 CLI（Go 1.25 + cobra，命令行发布）
└── README.md
```
...
| 调用方 | 被调用方 | 通信方式 | 数据格式 |
|--------|---------|---------|---------|
| aly-publish-gui | aly-publish | 子进程（`ProcessService`） | stdout JSON（`--json` 参数） |
| aly-publish | server | HTTP REST API | JSON（camelCase 统一包装） |
| client | server | HTTP REST API | JSON（camelCase 统一包装） |

aly-publish-gui 本身 **不直接调用 server API**，所有发布操作都通过子进程调用 aly-publish 完成。aly-publish-gui 仅在本地维护一个项目列表配置（`%LOCALAPPDATA%/AlyPublish/config.json`）。

**唯一例外**：添加项目时，aly-publish-gui 调用 `aly-publish project list` / `aly-publish project create` 操作服务端项目；通过 `aly-publish config init` 在选中的本地路径初始化 `.updator/` 配置（类比 SourceTree 的"克隆/添加仓库"时初始化 .git）。

---

## `.updator/` 共享配置体系

aly-publish 和 client 共用一套配置，存放在构建产物目录的 `.updator/` 文件夹中（类似 `.git` 的概念）。

### 目录结构

```
构建产物目录/                    ← aly-publish 的 project_path
├── .updator/
│   ├── shared.json             # aly-publish + client 共用配置
│   ├── publish.json            # aly-publish 专有配置（本地，不上传）
│   └── staging/
...
### publish.json（aly-publish 专有，本地不上传）
...
> `project_id` 不需要记录，因为 `shared.json` 中已有 `project_name`，aly-publish 通过项目名称查找即可定位 server 上的项目。

### 哪些文件会上传到 server

aly-publish 在扫描文件时自动忽略 `.updator/` 目录。此外，以下文件不会出现在 server 的文件清单中：

- `.updator/` 整个目录（scanner 自动跳过）
- `.aly-publish/` 整个目录（向后兼容，scanner 自动跳过）
...
> 1. **aly-publish 扫描** — 决定哪些文件被扫描并上传到 server
> 2. **client apply_update** — `CopyDirWithExclude` 时决定哪些文件/文件夹不复制到版本目录

### client.json（精简版）

位于 `aly-update.exe` 同级目录（UpdateFolder），包含定位 ApplicationFolder 和关闭进程所需的字段：
...
**aly-publish**：CLI 参数 > `.updator/shared.json` + `.updator/publish.json`
...
```bash
# Windows
aly-server-windows-amd64.exe -p 2000

# Linux / macOS
./aly-server-linux-amd64 -p 2000
```

首次启动会自动创建 SQLite 数据库（`configs/aly.db`）并建表。
...
# 3. 一键发布
aly-publish publish --version V1.0.1 --message "修复登录bug"
```

### 3. 配置客户端

在 `aly-update.exe` 同级目录创建 `client.json`（精简版）：

```yaml
main_exe_relative_path: "../ApplicationFolder/main_application.exe"
```

在 ApplicationFolder 中创建 `.updator/shared.json` 和 `.updator/client.json`（或通过 aly-publish 发布时自动带入）。
...
│   └── aly-update.exe               # updator 副本（用于自更新比对）
...
| `UpdateFolder/` | 部署时手动放置 | aly-update.exe 及其配置 |
...
类 Git 工作流的命令行工具，是整个发布流程的核心引擎。aly-publish-gui 通过子进程调用它完成所有发布操作。

### 构建

```bash
cd publish/aly-publish
go build -ldflags="-s -w" -o aly-publish.exe .
```
...
```bash
aly-publish config set server.url http://deploy.example.com
aly-publish config set output.format json
```
...
```bash
aly-publish config set-array ignore.folders --add ".cache"
aly-publish config set-array ignore.folders --remove ".cache"
aly-publish config set-array ignore.folders --clear
```
...
```bash
aly-publish config get server.url
```
...
```bash
aly-publish project list --server http://localhost:2000
```
...
```bash
aly-publish project update --name myapp --title "新标题" --force-update
```
...
```bash
aly-publish project delete --name myapp
```
...
```bash
aly-publish project info --name myapp
```
...
```bash
aly-publish status --project myapp
```

**实现**：
1. `resolveConfig()` 获取配置
2. `diff.RunStatus()` 执行核心比对：
   - `diff.ScanDirectory()` 递归扫描本地目录（自动跳过 `.updator/` 和 `.aly-publish/`，应用 ignore 规则），对每个文件同时计算 MD5 和 SHA256
...
```bash
aly-publish add app.exe lib.dll    # 添加指定文件
aly-publish add --all              # 添加所有 new + modified 文件
```
...
```bash
aly-publish reset app.exe     # 移除指定文件
aly-publish reset --all       # 清空暂存区
```
...
```bash
aly-publish push --version V1.0.1 --message "修复登录bug" --message "新增搜索"
```
...
```bash
aly-publish push-all --version V1.0.1 --message "快速修复"
```
...
```bash
aly-publish publish --version V1.0.1 --message "日常发布"
```
...
```bash
aly-publish log --project myapp --limit 5
```
...
```bash
aly-publish watch --interval 5 --auto-add
```
...
```bash
aly-publish server info --project myapp
```
...
1. 读取 `client.json`（与 `aly-update.exe` 同级目录）→ 获取 `main_exe_relative_path`、`must_close_process_name`
...
所有命令输出 JSON 到 stdout（与 aly-publish 格式统一）：
...
```bash
aly-update.exe check_update [--url <url>] [--project-name <name>]
```
...
```bash
aly-update.exe check_diff [--url <url>] [--project-name <name>]
```
...
```bash
aly-update.exe download_update [--url <url>] [--project-name <name>]
```
...
```bash
aly-update.exe apply_update [--main-exe-path <path>] [--close-timeout <seconds>]
```
...
```bash
aly-update.exe list_rollback_versions [--main-exe-path <path>]
```
...
```bash
aly-update.exe rollback --version 1.0.0 [--main-exe-path <path>] [--close-timeout <seconds>]
```
...
```bash
aly-update.exe check_self_update [--main-exe-path <path>]
```

**实现**：
1. 计算自身文件（`aly-update.exe`）的 SHA256
2. 检查 `ApplicationFolder/aly-update.exe` 是否存在（不存在 → 首次部署，无需自更新）
3. 计算 `aly-update.exe` 的 SHA256
...
version.json 位于 UpdateFolder（`aly-update.exe` 同级目录）：
...
## 发布 GUI（aly-publish-gui）

### 定位

面向发布者的桌面工具（Avalonia 12），纯前端壳子，所有发布操作通过子进程调用 aly-publish 完成。

### 构建

```bash
cd publish/aly-publish-gui
dotnet build
```
...
- 存储位置：`%LOCALAPPDATA%/AlyPublish/config.json`
...
**CliService** — aly-publish 命令封装

- 自动查找 `aly-publish.exe`（同级目录 → 相对路径回退）
...
|---------|---------------|----------------|
| **刷新** | `RefreshAsync()` | `aly-publish status --json` + `aly-publish log --limit 20 --json` |
| **全部暂存** | `AddAllAsync()` | `aly-publish add --all --json` |
| **清空暂存** | `ResetAllAsync()` | `aly-publish reset --all --json` |
| **暂存选中文件** | `AddSelectedAsync()` | `aly-publish add "file1" "file2" ... --json` |
| **取消暂存选中** | `ResetSelectedAsync()` | 先 `aly-publish reset --all --json`，再 `aly-publish add "保留文件1" ... --json` |
| **发布** | `PublishAsync()` | `aly-publish push --version "{ver}" --message "{msg}" --json`（timeout 120s） |
...
1. 用户启动 aly-publish-gui → 加载 `%LOCALAPPDATA%/AlyPublish/config.json` 中的项目列表
2. 选择项目 → 自动调 `status` + `log` 刷新数据
3. 左侧显示 unstaged 文件列表（new/modified/deleted）
4. 右侧显示 staged 文件列表
5. 用户勾选文件 → 点击"暂存选中" 或 "全部暂存"
6. 输入版本号和变更说明 → 点击"发布"
7. GUI 调 `push` → aly-publish 上传文件 + 创建版本 → 清空暂存区 → 自动刷新
...
{
    private readonly string _updatorPath; // aly-update.exe 的路径
...
public static class AppStartup
{
    ...
    {
        var updater = new UpdateManager("UpdateFolder/aly-update.exe");
...
aly-publish config set server.url $DEPLOY_SERVER
aly-publish config set project.name $PROJECT_NAME

cd ./dist && aly-publish publish \
...
```bash
cd server
go build -ldflags="-s -w" -o aly-server.exe .
```
...
CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -ldflags="-s -w" -o aly-server-linux-arm64 .
CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -ldflags="-s -w" -o aly-server-darwin-amd64 .
CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -ldflags="-s -w" -o aly-server-darwin-arm64 .
```
...
```bash
cd publish/aly-publish
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o aly-publish-linux-amd64 .
CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -ldflags="-s -w" -o aly-publish-linux-arm64 .
CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -ldflags="-s -w" -o aly-publish-darwin-amd64 .
CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -ldflags="-s -w" -o aly-publish-darwin-arm64 .
```
