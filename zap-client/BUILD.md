...
- 项目位于 GOPATH 下：
  - `zap-client/`     → `%GOPATH%/src/zap/zap-client/`
  - `zap-client-sdk/` → `%GOPATH%/src/zap/zap-client-sdk/`
...
```batch
rmdir /s /q %GOPATH%\src\zap\zap-client
rmdir /s /q %GOPATH%\src\zap\zap-client-sdk
```

### 2. 将工作目录的源码复制到 GOPATH

```batch
xcopy /e /i /y zap-client %GOPATH%\src\zap\zap-client
xcopy /e /i /y zap-client-sdk %GOPATH%\src\zap\zap-client-sdk
```
...
set GOARCH=386
set GO111MODULE=off
go build -ldflags="-s -w" -o zap-update.exe zap/zap-client
```
...
```batch
copy /y zap-update.exe zap-client\zap-update.exe
del zap-update.exe
```
...
echo 正在复制源码到 GOPATH...
xcopy /e /i /y zap-client %GOPATH%\src\zap\zap-client >nul
xcopy /e /i /y zap-client-sdk %GOPATH%\src\zap\zap-client-sdk >nul
...
copy /y zap-update.exe zap-client\zap-update.exe >nul
del zap-update.exe

echo 构建成功：zap-client\zap-update.exe
```
...
检查是否用了绝对路径作为 package 参数（如 `C:\Users\xxx`）。Go 1.10 GOPATH 模式下，绝对路径会被拼接到 `$GOROOT/src/` 和 `$GOPATH/src/` 后面，导致找不到包或编译的是旧文件。应始终使用 import path `zap/zap-client`。
