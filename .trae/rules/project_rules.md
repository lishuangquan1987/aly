...
- 全局样式在 `App.axaml` 中通过 `FluentTheme` 配置
- 使用 `UseAtomUI()` 扩展方法在 `App.axaml.cs` 的 `OnInitialized()` 中启用 AtomUI
- 不使用内联样式
- 颜色引用 `{DynamicResource}` 资源键，不使用硬编码颜色值
- 对话框使用单独的 Window，不弹模态时用 `ShowDialog()`
